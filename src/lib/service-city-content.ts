...
const ANGLES: Record<ServiceSlug, ServiceCityAngle> = {
  ...
  "viyskovyi-advokat": {
    lead: (c) =>
      `Військовий адвокат ${c}: захист у справах ТЦК, ВЛК, СЗЧ, спори з командуванням і виплати. Перша консультація безкоштовна.`,
    valuePara:
      "Військовий адвокат LEGAR — персональний адвокат-партнер НААУ, який веде всі ваші військово-правові питання разом: ТЦК, ВЛК, СЗЧ, оскарження рішень командування, звільнення, виплати. Одна стратегія замість розрізнених консультацій.",
    localPara: (city) =>
      `${city.nameUk}: адвокат супроводжує справи по районах (${districtList(city)}) — дистанційно, а за потреби з виїздом за ${city.lawyerVisit.reactionTck}. Первинна консультація доступна онлайн з будь-якої точки регіону.`,
    faqQuestion: (c) => `Скільки коштує військовий адвокат ${c}?`,
    faqAnswer: (city) =>
      `Перша консультація ${city.nameInflected} — безкоштовна. Ведення окремого питання — від 6 000 грн, комплексний супровід — від 20 000 грн. Точну ціну називаємо після оцінки.`,
  },
  "b2b-bronyuvannya": {
    lead: (c) =>
